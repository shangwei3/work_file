...
static void key_work(struct work_struct *work_s)
{
	struct key_device *pkey = container_of(work_s,struct key_device,key_work);
	printk("key work sucess,irq : %d\n",pkey->irq);
...
static int key_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&pkey->key_work,key_work);
