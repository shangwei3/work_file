...
void of_parse_led_dt(struct device_node *np,struct led_device *pled)
{	
	of_property_read_u32(np, "pin", &pled->pin);
	of_property_read_u32(np, "mode", &pled->mode);
	of_property_read_u32(np, "level",&pled->level);
...
static int led_probe(struct platform_device *pdev)
{
	...
	if(pd == NULL){
		//设备树的形式，提供私有信息，就需要自己去解析设备树
		//获取私有信息
		of_parse_led_dt(pdev->dev.of_node,pled);
	}else{
		pled->pin  = pd->pin;
		pled->mode = pd->mode;
