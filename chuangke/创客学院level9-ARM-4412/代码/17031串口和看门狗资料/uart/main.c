...
int uart2_init()
{
	...
	reg_write(ULCON2,0x3);			 //设置数据宽度为 8位 \1位停止位\没有校验位\正常模式 
	reg_write(UCON2,(reg_read(UCON2)&(~0Xf))|0X5);  //收发模式 01 = Interrupt request or polling mode
	reg_write(UFCON2,reg_read(UFCON2)&(~1));		 //关闭FIFO模式
	//reg_write(UMCON,reg_read(UMCON)|0X1);        //关闭流控
	/*while(1){
	 reg_write(GPX1CON,(reg_read(GPX1CON)&(~(0XF)))|(0X1));
     reg_write(GPX1DAT,reg_read(GPX1DAT)|(0X1));} */	
	return 0;
