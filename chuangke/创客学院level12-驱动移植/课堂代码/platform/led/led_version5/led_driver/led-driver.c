...
/*
	bug:
		不论操作哪一个设备文件，最终都是操作了最后一个LED灯

	原因:
		pled这个指针是全局变量，而它的值最后一次匹配的值(最后一个LED匹配)

	解决问题方法：
	必须找到每个LED灯对应的struct led_device结构体

	platform_set_drvdata / platform_get_drvdata
	container_of : 根据结构体成员地址，算出结构体首地址
	struct file的private_data成员保存结构体首地址
 */
...
int led_driver_init(void)
{
	...
	if (IS_ERR(led_class)){//判断是是否出错
		printk("Fail to class create\n");
		return PTR_ERR(led_class);//获取错误码
	}
